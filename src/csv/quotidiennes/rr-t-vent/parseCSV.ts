...
const quotidienneLineSchema = z.object({
    ...
    QHTN: z.string().transform(toCodeQualite), // 9
    ...
    TNTXM: z.string().transform(toFloatOrNull), // -2.2
    QTNTXM: z.string().transform(toCodeQualite), // 9

    // TAMPLI      : amplitude thermique quotidienne : écart entre TX et TN quotidiens (TX-TN) (en °C et 1/10)
    TAMPLI: z.string().transform(toPositiveFloat), // 1.1
    QTAMPLI: z.string().transform(toCodeQualite), // 9

    // TNSOL       : température quotidienne minimale à 10 cm au-dessus du sol (en °C et 1/10)
    TNSOL: z.string().transform(toFloatOrNull), // -2.2
    QTNSOL: z.string().transform(toCodeQualite), // 9

    // TN50        : température quotidienne minimale à 50 cm au-dessus du sol (en °C et 1/10)
    TN50: z.string().transform(toFloatOrNull), // -2.2
    QTN50: z.string().transform(toCodeQualite), // 9

    // DG          : durée de gel sous abri (T ≤ 0°C) (en mn)
    DG: z.string().transform(toPositiveInteger), // 3
    QDG: z.string().transform(toCodeQualite), // 9
    ...
    QDXI3S: z.string().transform(toCodeQualite), // 9

    // HXI3S       : heure de FXI3S (hhmm)
    HXI3S: z.string().transform(toTime), // 1230
    QHXI3S: z.string().transform(toCodeQualite), // 9

    // DRR         : durée des précipitations (en mn)
    DRR: z.string().transform(toPositiveInteger), // 3
    QDRR: z.string().transform(toCodeQualite), // 9
});
